...
object KafkaSinkConnectorNonBlocking{

  var counter = 0
  lazy val props: Properties = new Properties
  var kafkaProducer: KafkaProducer[Object, Object] = null
  private final val logger1: Logger = LoggerFactory.getLogger(classOf[KafkaSinkConnectorNonBlocking])
...
class KafkaSinkConnectorNonBlocking extends SinkConnector{

  private final val logger: Logger = LoggerFactory.getLogger(classOf[KafkaSinkConnectorNonBlocking])
  ...
  override def start(props: util.Map[String, String]): Unit = {
    ...
    //this.kafkaProducer =
    KafkaSinkConnectorNonBlocking.registerProducerProperties(producerConfig, bootstrapServers, schemaRegistryUrl)
  ...
  override def taskClass(): Class[_ <: Task] = classOf[KafkaSinkTaskNonBlocking]
