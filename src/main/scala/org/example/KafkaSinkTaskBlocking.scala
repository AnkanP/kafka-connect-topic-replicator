...
class KafkaSinkTaskBlocking extends SinkTask{

  private val log: Logger = LoggerFactory.getLogger(classOf[KafkaSinkTaskBlocking])
...
//  }
  ...
  override def start(map: util.Map[String, String]): Unit = {
    ...
    try{
      if (KafkaSinkConnectorBlocking.getKafkaProducerInstance == null) {
  ...
  override def put(collection: util.Collection[SinkRecord]): Unit = {
    ...
    for (record <- collection) {
      ...
      try{
        ...
        val metadata = KafkaSinkConnectorBlocking.getKafkaProducerInstance.send(producerRecord).get()
        log.info(s"Produced record at @topic: " + metadata.topic() + ", partition=" + metadata.partition() + ", offset=" +metadata.offset().toString)

        //reset counter on success
        //initializationObject.remainingRetries =  maxretrues

      } catch {
        case e: Exception =>
          //KafkaSinkConnectorBlocking.deregisterKafkaProducerInstance()
          ...
          //super.context.timeout(initializationObject.retryBackoffMs)
...
//context.timeout(30)
  ...
  override def version(): String = {
    new KafkaSinkConnectorBlocking().version()
  }
  ...
  //  try {
  //    KafkaSinkConnectorBlocking.getKafkaProducerInstance.flush()
  //  } catch {
  //    case e: Exception =>
  //      throw new RuntimeException(e.getMessage)
  //  }
